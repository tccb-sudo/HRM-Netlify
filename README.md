...
Frontend chạy trên Cloudflare Pages. Pages Function `/api/apps-script` chuyển tiếp yêu cầu đến Apps Script. Apps Script tiếp tục giữ toàn bộ nghiệp vụ, Supabase service-role key và GmailApp.
...
Vào **Workers & Pages → dự án → Settings → Variables and Secrets** và thêm cho môi trường Production:
...
## 3. Đưa mã lên Cloudflare Pages

Vì hệ thống có Pages Function, không triển khai bằng cách chỉ tải riêng `index.html`. Hãy triển khai qua Git:
...
5. Trong Cloudflare → dự án → Functions/Logs, kiểm tra route `/api/apps-script` nếu có lỗi.
6. Trong Apps Script → Executions, kiểm tra log `[PERF]`.

## 5. Tên miền riêng

Cloudflare Pages → **Custom domains → Set up a custom domain**. Không thay đổi mã khi đổi từ `*.pages.dev` sang tên miền riêng vì frontend dùng đường dẫn cùng miền `/api/apps-script`.
