...
function finishLogin_(emp){var scopedRows=scopedAcademicRequests_(emp),ownInfo=leaveInfoFromRows_(scopedRows.filter(function(r){return r.cccd===emp.cccd;}),emp.annual_leave_days);emp.used_leave_days=ownInfo.used;var token=Utilities.getUuid().replace(/-/g,'')+Utilities.getUuid().replace(/-/g,'');CacheService.getScriptCache().put('TOKEN_'+token,JSON.stringify({employee:emp}),Math.floor(CONFIG.TOKEN_TTL_MS/1000));log_('LOGIN',emp.cccd,'role='+emp.role+'; org='+emp.org_id);var clean=sanitizeEmp_(emp),visible=filterRowsForRole_(scopedRows,emp,'').map(requestView_);return ok_({token:token,user:clean,bootstrap:{user:clean,requests:visible,stats:canApprove_(emp.role)?statsFromRows_(visible):null,holidays:isLecturer_(emp.role)?getHolidays_(new Date().getFullYear()):[]}});}
function verify_(token){if(!token)return null;var raw=CacheService.getScriptCache().get('TOKEN_'+token);if(!raw)return null;var t=JSON.parse(raw),old=t.employee;if(!old)return null;return buildEmployeeV2_(old.cccd,old.org_id)||old;}
...
function saveOrganization_(token,d){var me=verify_(token);if(!me)return fail_('Chưa đăng nhập');if(!isHR_(me.role))return fail_('Không có quyền');var parent=resolveOrg_(d.parent_id);if(!parent||!canManageOrg_(me,parent.id))return fail_('Không có quyền tạo đơn vị tại đây');var id=String(d.id||('org-'+Utilities.getUuid().slice(0,8))).trim(),code=String(d.code||id).trim().toUpperCase(),name=String(d.name||'').trim(),type=String(d.org_type||'department');if(!name)return fail_('Vui lòng nhập tên đơn vị');insert_('organizations',{id:id,code:code,name:name,org_type:type,parent_id:parent.id,active:d.active!==false,sort_order:Number(d.sort_order)||100,updated_at:new Date().toISOString()},true);CacheService.getScriptCache().remove('ORG_TREE_V2');log_('SAVE_ORG',me.cccd,id);return ok_({message:'Đã lưu đơn vị',org_id:id});}
