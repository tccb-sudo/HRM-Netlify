...
function login_(cccd) {
  cccd = normalizeCCCD_(cccd);
  if (!/^\d{12}$/.test(cccd)) return fail_('Vui lòng nhập đủ 12 số CCCD');
  var emp = findEmployeeFast_(cccd);
  if (!emp) return fail_('CCCD không tồn tại. Liên hệ phòng nhân sự.');
  if (isManager_(emp.role) && emp.departments.length > 1) return ok_({ needSelectDept:true, departments:emp.departments, name:emp.name, cccd:emp.cccd });
  return finishLogin_(emp);
}
function loginWithDept_(cccd, dept) {
  var emp = findEmployeeFast_(normalizeCCCD_(cccd));
  if (!emp || emp.departments.indexOf(dept) < 0) return fail_('Đơn vị không hợp lệ');
  emp.department = dept;
  return finishLogin_(emp);
}
function finishLogin_(emp) {
  var scopedRows = scopedAcademicRequests_(emp);
  var ownInfo = leaveInfoFromRows_(scopedRows.filter(function(r) { return r.cccd === emp.cccd; }), emp.annual_leave_days);
  emp.used_leave_days = ownInfo.used;
  var token = Utilities.getUuid().replace(/-/g, '') + Utilities.getUuid().replace(/-/g, '');
  CacheService.getScriptCache().put('TOKEN_' + token, JSON.stringify({cccd:emp.cccd, department:emp.department, employee:emp}), Math.floor(CONFIG.TOKEN_TTL_MS / 1000));
  log_( 'LOGIN', emp.cccd, 'role=' + emp.role + '; department=' + emp.department);
  var cleanUser = sanitizeEmp_(emp);
  var visibleRows = filterRowsForRole_(scopedRows, emp, '').map(requestView_);
  return ok_({ token:token, user:cleanUser, bootstrap:{
    user:cleanUser,
    requests:visibleRows,
    stats:canApprove_(emp.role) ? statsFromRows_(filterRowsForRole_(scopedRows, emp, '')) : null,
    holidays:isLecturer_(emp.role) ? getHolidays_(new Date().getFullYear()) : []
  }});
}
function verify_(token) {
  ...
  if (t.employee) return t.employee;
  var emp = findEmployee_(t.cccd);
  if (emp && t.department && emp.departments.indexOf(t.department) >= 0) emp.department = t.department;
  return emp;
}
function me_(token) {var e=verify_(token);if(!e)return fail_('Token không hợp lệ hoặc hết hạn');var info=detailedLeave_(e.cccd,e.annual_leave_days);e.used_leave_days=info.used;CacheService.getScriptCache().put('TOKEN_'+token,JSON.stringify({cccd:e.cccd,department:e.department,employee:e}),Math.floor(CONFIG.TOKEN_TTL_MS/1000));return ok_({user:sanitizeEmp_(e)});}
...
function findEmployee_(cccd) {
  var rows = select_('employees', {cccd:normalizeCCCD_(cccd)});
  if (!rows.length) return null;
  var r = rows[0], depts = [];
  rows.forEach(function(x) { if (x.department && depts.indexOf(x.department) < 0) depts.push(x.department); });
  var annual = calcAnnual_(r.start_date), info = detailedLeave_(r.cccd, annual);
  return { cccd:r.cccd, name:r.name, department:r.department, departments:depts, role:r.role, email:r.email || '', start_date:r.start_date || '', annual_leave_days:annual, used_leave_days:info.used, seniority_years:seniority_(r.start_date) };
}
function findEmployeeFast_(cccd) {
  var rows = select_('employees', {cccd:normalizeCCCD_(cccd)});
  if (!rows.length) return null;
  var r=rows[0], depts=[];
  rows.forEach(function(x){ if(x.department && depts.indexOf(x.department)<0) depts.push(x.department); });
  return {cccd:r.cccd,name:r.name,department:r.department,departments:depts,role:r.role,email:r.email||'',start_date:r.start_date||'',annual_leave_days:calcAnnual_(r.start_date),used_leave_days:0,seniority_years:seniority_(r.start_date)};
}
function sanitizeEmp_(e) { var ay=academicPeriod_(new Date()); return {cccd:e.cccd,name:e.name,department:e.department,departments:e.departments,role:e.role,email:e.email,start_date:e.start_date,annual_leave_days:e.annual_leave_days,used_leave_days:e.used_leave_days,remaining_leave_days:e.annual_leave_days-e.used_leave_days,seniority_years:e.seniority_years,academic_year:ay.label,academic_year_start:dateISO_(ay.start),academic_year_end:dateISO_(ay.end),is_lecturer:isLecturer_(e.role),is_manager:isManager_(e.role),is_hr:isHR_(e.role)}; }
function getEmployees_(token) {
  ...
  var rows = select_('employees', {}, 'name.asc');
  var leaveRows=scopedAcademicRequests_(me),byCCCD={};
  leaveRows.forEach(function(r){(byCCCD[r.cccd]=byCCCD[r.cccd]||[]).push(r);});
  return ok_({employees:rows.map(function(r){var annual=calcAnnual_(r.start_date),info=leaveInfoFromRows_(byCCCD[r.cccd]||[],annual);return{cccd:r.cccd,name:r.name,department:r.department,role:r.role,email:r.email||'',start_date:r.start_date||'',annual_leave_days:annual,used_leave_days:info.used,remaining_leave_days:info.remaining};})});
}
function createEmployee_(token, d) {
  var me=verify_(token); if(!me)return fail_('Chưa đăng nhập'); if(!isHR_(me.role))return fail_('Không có quyền');
  var c=normalizeCCCD_(d.cccd); if(!/^\d{12}$/.test(c)||!d.name||!d.department||!d.role)return fail_('Thiếu hoặc sai thông tin nhân viên');
  if(one_('employees',{cccd:c,department:d.department}))return fail_('CCCD đã tồn tại trong đơn vị này');
  insert_('employees',{cccd:c,name:d.name,department:d.department,role:d.role,email:d.email||'',start_date:dateISO_(d.start_date)});
  log_('CREATE_EMP',me.cccd,c); return ok_({message:'Thêm nhân viên thành công'});
}
function updateEmployee_(token,d) {
  var me=verify_(token); if(!me)return fail_('Chưa đăng nhập'); if(!isHR_(me.role))return fail_('Không có quyền');
  var c=normalizeCCCD_(d.cccd), patch={}; ['name','department','role','email'].forEach(function(k){if(d[k]!==undefined)patch[k]=d[k];}); if(d.start_date!==undefined)patch.start_date=dateISO_(d.start_date);
  if(!Object.keys(patch).length)return fail_('Không có dữ liệu cập nhật');
  var out=update_('employees',{cccd:c},patch); if(!out||!out.length)return fail_('Không tìm thấy nhân viên'); log_('UPDATE_EMP',me.cccd,c); return ok_({message:'Cập nhật thành công'});
}
function importEmployees_(token,d) {
  var me=verify_(token);if(!me)return fail_('Chưa đăng nhập');if(!isHR_(me.role))return fail_('Không có quyền');
  var input=d.employees;if(!Array.isArray(input)||!input.length)return fail_('Tệp import không có dữ liệu');if(input.length>2000)return fail_('Mỗi lần chỉ import tối đa 2.000 dòng');
  var validRoles=['employee','lecturer','manager','manager_lecturer','hr'],rows=[],errors=[];
  input.forEach(function(r,i){var line=i+2,c=normalizeCCCD_(r.cccd),role=String(r.role||'').trim(),name=String(r.name||'').trim(),dept=String(r.department||'').trim();if(!/^\d{12}$/.test(c)||!name||!dept||validRoles.indexOf(role)<0){errors.push('Dòng '+line+': CCCD/họ tên/đơn vị/vai trò không hợp lệ');return;}rows.push({cccd:c,name:name,department:dept,role:role,email:String(r.email||'').trim(),start_date:dateISO_(r.start_date)});});
  if(errors.length)return fail_('Không thể import. '+errors.slice(0,10).join('; ')+(errors.length>10?' ...':''));
  for(var i=0;i<rows.length;i+=200)insert_('employees',rows.slice(i,i+200),true);
  log_('IMPORT_EMPLOYEES',me.cccd,'rows='+rows.length);return ok_({message:'Đã import '+rows.length+' dòng nhân viên',imported:rows.length});
}
...
function createLeaveRequest_(token,d) {
  ...
  var leaveYear=academicPeriod_(from);if(to>leaveYear.end)return fail_('Đơn nghỉ không được kéo dài qua hai năm học. Năm học '+leaveYear.label+' kết thúc ngày 30/06/'+leaveYear.end.getFullYear()+'.');
  ...
  var targetInfo=detailedLeave_(e.cccd,e.annual_leave_days,from),remain=targetInfo.remaining;if(days>remain)return fail_('Vượt số ngày phép còn lại ('+remain+' ngày) trong năm học '+leaveYear.label);
  ...
  insert_('leave_requests',{id:id,cccd:e.cccd,employee_name:e.name,department:e.department,from_date:dateISO_(from),to_date:dateISO_(to),days:days,reason:String(d.reason).trim(),status:'pending',attachment:d.attachment||'',history:history,half_day:half,return_info:null,created_at:now,updated_at:now});
...
function scopedAcademicRequests_(e) {
  ...
  if(e.role==='employee'||e.role==='lecturer') query+='&cccd=eq.'+q_(e.cccd);
...
function filterRowsForRole_(rows,e,scope) {
  ...
  if(scope==='team') {
    if(isManager_(e.role)) return rows.filter(function(r){return e.departments.indexOf(r.department)>=0&&r.cccd!==e.cccd;});
    if(isHR_(e.role)) return rows.filter(function(r){return r.cccd!==e.cccd;});
    return [];
  }
  if(isManager_(e.role)) return rows.filter(function(r){return e.departments.indexOf(r.department)>=0;});
  return rows;
}
...
function getLeaveRequests_(token,d) {
  var e=verify_(token);if(!e)return fail_('Chưa đăng nhập');var rows=scopedAcademicRequests_(e).map(requestView_);var scope=String(d.scope||'');
  if(scope==='mine'||(!scope&&(e.role==='employee'||e.role==='lecturer')))rows=rows.filter(function(r){return r.cccd===e.cccd;});
  else if(scope==='team'){if(isManager_(e.role))rows=rows.filter(function(r){return e.departments.indexOf(r.department)>=0&&r.cccd!==e.cccd;});else if(isHR_(e.role))rows=rows.filter(function(r){return r.cccd!==e.cccd;});else rows=[];}
  else if(isManager_(e.role))rows=rows.filter(function(r){return e.departments.indexOf(r.department)>=0;});
...
function changeStatus_(token,d,status) {
  ...
  if(status==='approved'||status==='rejected'){if(!isManager_(e.role)&&!isHR_(e.role))return fail_('Không có quyền');if(isManager_(e.role)&&e.departments.indexOf(req.department)<0)return fail_('Đơn không thuộc đơn vị phụ trách');if(status==='rejected'&&!note)return fail_('Vui lòng nhập lý do từ chối');if(req.status!=='pending')return fail_('Đơn đã được xử lý');}
  else {if(!isHR_(e.role))return fail_('Chỉ Nhân sự phụ trách mới có quyền');if(status==='hr_confirmed'&&req.status!=='approved')return fail_('Đơn chưa được trưởng đơn vị duyệt');if(status==='hr_returned'&&!note)return fail_('Vui lòng nhập lý do trả lại');}
  var hist=req.history||[];hist.push(historyItem_(e,status,note));var patch={status:status,history:hist,updated_at:new Date().toISOString()};
  ...
  update_('leave_requests',{id:req.id},patch);log_(status.toUpperCase(),e.cccd,req.id);
...
function resolveReturn_(token,d,confirm) {
  var e=verify_(token);if(!e)return fail_('Chưa đăng nhập');if(!isHR_(e.role))return fail_('Chỉ Nhân sự phụ trách mới có quyền');var r=getRequest_(d.request_id);if(!r||!r.return_info)return fail_('Không tìm thấy yêu cầu trả phép');var info=r.return_info;if(info.status!=='return_pending')return fail_('Yêu cầu này đã được xử lý');if(!confirm&&!String(d.note||'').trim())return fail_('Vui lòng nhập lý do từ chối');
  var now=new Date().toISOString();info.status=confirm?'return_confirmed':'return_rejected';info.hr_note=d.note||'';info.confirmed_at=now;info.confirmed_by=e.cccd;var hist=r.history||[];hist.push(historyItem_(e,info.status,(confirm?'Xác nhận':'Từ chối')+' trả phép '+info.return_days+' ngày. '+(d.note||'')));var patch={return_info:info,history:hist,updated_at:now};
...
function getEmployeeLeaveInfo_(token,d){var e=verify_(token);if(!e)return fail_('Chưa đăng nhập');if(!canApprove_(e.role))return fail_('Không có quyền');var t=findEmployee_(d.cccd);if(!t)return fail_('Không tìm thấy nhân viên');var info=detailedLeave_(t.cccd,t.annual_leave_days);return ok_({cccd:t.cccd,name:t.name,department:t.department,annual_days:t.annual_leave_days,used_days:info.used,remaining:info.remaining,seniority:t.seniority_years,start_date:t.start_date,academic_year:info.academic_year,period_start:info.period_start,period_end:info.period_end,requests:info.requests});}
...
function managers_(dept){return select_('employees',{department:dept}).filter(function(e){return isManager_(e.role)&&e.email;});}function hrs_(){return select_('employees',{}).filter(function(e){return isHR_(e.role)&&e.email;});}
function notifyManagerNew_(e,id,from,to,days,reason,half){managers_(e.department).forEach(function(m){sendMail_(m.email,'[Cần duyệt] Đơn xin nghỉ phép của '+e.name,'Đơn xin nghỉ phép mới',[['VC-NLĐ',e.name],['Đơn vị',e.department],['Mã đơn',id],['Từ ngày',from],['Đến ngày',to],['Số ngày',String(days)],['Lý do',reason]],'Đăng nhập '+appUrl_()+' để xem xét.');});}
function notifyHRPending_(r){hrs_().forEach(function(h){sendMail_(h.email,'[Chờ xác nhận] Đơn nghỉ phép của '+r.employee_name,'Đơn chờ Nhân sự phụ trách xác nhận',[['VC-NLĐ',r.employee_name],['Đơn vị',r.department],['Mã đơn',r.id],['Số ngày',String(r.days)]],'Đăng nhập '+appUrl_()+' để xác nhận.');});}
...
function notifyHRReturn_(e,id,info){hrs_().forEach(function(h){sendMail_(h.email,'[Trả phép] '+e.name+' muốn trả phép','Yêu cầu trả phép',[['Nhân viên',e.name],['Đơn vị',e.department],['Mã đơn',id],['Số ngày trả',String(info.return_days)],['Lý do',info.reason]],'Đăng nhập '+appUrl_()+' để xử lý.');});}
