...
function route_(action, d, token) {
  try {
    switch (action) {
      ...
      case 'import-organizations': return importOrganizations_(token, d);
      ...
      case 'evaluation-context': return evaluationContextApi_(token);
      case 'evaluation-status': return evaluationStatus_(token,d);
      case 'evaluation-submit': return evaluationSubmit_(token,d);
      case 'evaluation-history': return evaluationHistory_(token);
      case 'evaluation-unit-data': return evaluationScopedData_(token,d,'manager');
      case 'evaluation-hr-data': return evaluationScopedData_(token,d,'hr');
      case 'evaluation-manager-save': return evaluationManagerSave_(token,d);
      case 'evaluation-export': return evaluationExport_(token,d);
...
function resolveOrg_(value){var s=String(value||'').trim(),idx=orgIndex_(),byName=[];for(var i=0;i<idx.rows.length;i++){var o=idx.rows[i];if(o.id===s||String(o.code).toUpperCase()===s.toUpperCase())return o;if(o.name===s)byName.push(o);}return byName.length===1?byName[0]:null;}
function orgPathLabel_(orgId,idx){idx=idx||orgIndex_();var names=[],seen={},id=orgId;while(id&&idx.map[id]&&!seen[id]){seen[id]=true;names.unshift(idx.map[id].name);id=idx.map[id].parent_id;}return names.join(' › ');}
...
function getEmployees_(token){
  ...
  memberships.forEach(function(m){
    ...
    out.push({cccd:p.cccd,name:p.name,department:o?o.name:m.org_id,department_path:orgPathLabel_(m.org_id,idx),org_code:o?o.code:'',org_id:m.org_id,role:role,worker_type:m.worker_type,email:p.email||'',start_date:p.start_date||'',annual_leave_days:annual,used_leave_days:used,reserved_leave_days:reserved,remaining_leave_days:Math.max(0,annual-used-reserved)});
  });
  out.sort(function(a,b){return String(a.name).localeCompare(String(b.name),'vi');});
  return ok_({employees:out});
}
function savePersonMembership_(d,upsert){var c=normalizeCCCD_(d.cccd),org=resolveOrg_(d.org_id||d.org_code||d.department),worker=['lecturer','manager_lecturer'].indexOf(d.role)>=0?'lecturer':(d.worker_type||'employee');if(!org)throw new Error('Không tìm thấy đơn vị hoặc tên đơn vị bị trùng; hãy dùng org_code');insert_('people',{cccd:c,name:String(d.name||'').trim(),email:String(d.email||'').trim(),start_date:dateISO_(d.start_date),active:true},true);var existing=select_('organization_memberships',{cccd:c,active:true}),makePrimary=d.is_primary===true||!existing.length;if(d.is_primary===true)update_('organization_memberships',{cccd:c},{is_primary:false});insert_('organization_memberships',{cccd:c,org_id:org.id,worker_type:worker,is_primary:makePrimary,active:true},true);if(d.role==='manager'||d.role==='manager_lecturer')upsertOn_('role_assignments',{cccd:c,org_id:org.id,role:'unit_manager',include_descendants:false,active:true},['cccd','org_id','role']);else update_('role_assignments',{cccd:c,org_id:org.id,role:'unit_manager'},{active:false});if(d.role==='hr')upsertOn_('role_assignments',{cccd:c,org_id:org.id,role:'organization_hr',include_descendants:true,active:true},['cccd','org_id','role']);return org;}
function createEmployee_(token,d){var me=verify_(token);if(!me)return fail_('Chưa đăng nhập');if(!isHR_(me.role))return fail_('Không có quyền');var c=normalizeCCCD_(d.cccd),org=resolveOrg_(d.org_id||d.org_code||d.department);if(!/^\d{12}$/.test(c)||!d.name||!org||!d.role)return fail_('Thiếu hoặc sai thông tin nhân viên/đơn vị');if(!canManageOrg_(me,org.id))return fail_('Đơn vị nằm ngoài phạm vi quản lý');savePersonMembership_(d,true);log_('CREATE_EMP',me.cccd,c+'@'+org.id);return ok_({message:'Thêm nhân viên thành công'});}
function updateEmployee_(token,d){var me=verify_(token);if(!me)return fail_('Chưa đăng nhập');if(!isHR_(me.role))return fail_('Không có quyền');var c=normalizeCCCD_(d.cccd),org=resolveOrg_(d.org_id||d.org_code||d.department);if(!org||!canManageOrg_(me,org.id))return fail_('Đơn vị không hợp lệ hoặc ngoài phạm vi quản lý');savePersonMembership_(d,true);log_('UPDATE_EMP',me.cccd,c+'@'+org.id);return ok_({message:'Cập nhật thành công'});}
function importEmployees_(token,d){var me=verify_(token);if(!me)return fail_('Chưa đăng nhập');if(!isHR_(me.role))return fail_('Không có quyền');var input=d.employees;if(!Array.isArray(input)||!input.length)return fail_('Tệp import không có dữ liệu');if(input.length>2000)return fail_('Mỗi lần chỉ import tối đa 2.000 dòng');var errors=[],done=0;input.forEach(function(r,i){try{var c=normalizeCCCD_(r.cccd),org=resolveOrg_(r.org_id||r.org_code);if(!/^\d{12}$/.test(c)||!r.name||!org||!canManageOrg_(me,org.id))throw new Error('CCCD hoặc org_code không hợp lệ/ngoài phạm vi');savePersonMembership_(r,true);done++;}catch(err){errors.push('Dòng '+(i+2)+': '+err.message);}});log_('IMPORT_EMPLOYEES',me.cccd,'ok='+done+', errors='+errors.length);return ok_({message:errors.length?'Đã nhập '+done+'/'+input.length+' nhân viên. Lỗi: '+errors.slice(0,10).join('; '):'Đã import '+done+' nhân viên',imported:done,failed:errors.length,errors:errors});}
...
function importOrganizations_(token,d){var me=requireAdmin_(token);if(!me)return fail_('Chỉ Quản trị hệ thống được import đơn vị');var rows=d.organizations;if(!Array.isArray(rows)||!rows.length)return fail_('Tệp import không có dữ liệu');if(rows.length>1000)return fail_('Mỗi lần chỉ import tối đa 1.000 đơn vị');var validTypes=['university','school','faculty','center','office','department','division','subject'],idx=orgIndex_(),byCode={},errors=[],done=0;idx.rows.forEach(function(o){byCode[String(o.code).toUpperCase()]=o;});rows.forEach(function(r,i){try{var code=String(r.code||'').trim().toUpperCase(),name=String(r.name||'').trim(),type=String(r.org_type||'').trim().toLowerCase(),parentCode=String(r.parent_code||'').trim().toUpperCase();if(!code||!name||validTypes.indexOf(type)<0)throw new Error('thiếu mã/tên hoặc loại đơn vị không hợp lệ');var existing=byCode[code]||null,parent=parentCode?byCode[parentCode]:null;if(parentCode&&!parent)throw new Error('không tìm thấy đơn vị cha '+parentCode+'; hãy đặt dòng cha phía trên');if(!existing&&!parent)throw new Error('đơn vị mới phải có mã đơn vị cha');if(existing&&existing.code==='UMP'&&parent)throw new Error('đơn vị gốc UMP không được có đơn vị cha');if(parent&&existing&&(parent.id===existing.id||descendantOrgIds_(existing.id,idx).indexOf(parent.id)>=0))throw new Error('đơn vị cha tạo vòng lặp');var id=existing?existing.id:'org-'+Utilities.getUuid().slice(0,8),obj={id:id,code:code,name:name,org_type:type,parent_id:parent?parent.id:null,active:true,sort_order:Number(r.sort_order)||100,updated_at:new Date().toISOString()};insert_('organizations',obj,true);byCode[code]=obj;idx.rows.push(obj);idx.map[id]=obj;done++;}catch(err){errors.push('Dòng '+(i+2)+': '+err.message);}});CacheService.getScriptCache().remove('ORG_TREE_V2');log_('IMPORT_ORGANIZATIONS',me.cccd,'ok='+done+', errors='+errors.length);return ok_({message:errors.length?'Đã nhập '+done+'/'+rows.length+' đơn vị. Lỗi: '+errors.slice(0,10).join('; '):'Đã import '+done+' đơn vị',imported:done,failed:errors.length,errors:errors});}
