...
```json
{"success":true,"service":"HRM Apps Script Middleware","version":"HRM_ENTERPRISE_2.1.0"}
```

## 5. Cập nhật GitHub và Cloudflare Pages

Upload toàn bộ mã nguồn lên GitHub. Cloudflare Pages tự nhận Pages Function tại `functions/api/apps-script.js`.

Endpoint kiểm tra:

```text
https://TEN-DU-AN.pages.dev/api/apps-script
```

Kết quả GET đúng là HTTP 405 với `HRM_NETLIFY_PROXY_2.1.0`.

## 5A. Số dư phép theo năm học

Sau schema Enterprise V2, chạy thêm `schema-v2.1-leave-balances.sql`. Mỗi năm học có một dòng số dư độc lập; `carryover_days` luôn bằng 0. Đơn chờ duyệt được ghi vào `reserved_days`, HR xác nhận mới chuyển sang `used_days`.
